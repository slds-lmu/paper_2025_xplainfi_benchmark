...
As suggested by a reviewer, the plots below instead show the distribution of the *paired difference* in importance between \CRANpkg{xplainfi} and each reference implementation, where both scores are computed within the same replication, i.e., on identical train-test splits.
For each feature, method, learner, and replication, we subtract the reference package's importance from \CRANpkg{xplainfi}'s; boxplots centered on the dashed line at zero therefore indicate agreement on a given dataset, which is more informative than comparing marginal distributions.
